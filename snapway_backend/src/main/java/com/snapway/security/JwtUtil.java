...
public class JwtUtil {
	...
	public String generateAccessToken(Member member, int userId, String email, List<String> roles) {
		...
		return Jwts.builder().subject(email).claim("userId", userId).claim("realName", member.getUsername())
				.claim("roles", roles).claim("type", "access").issuedAt(now).expiration(expire)
	...
	public String generateRefreshToken(String username) {
		...
		return Jwts.builder().subject(username).claim("type", "refresh").issuedAt(now)
				.expiration(expire).signWith(secretKey).compact();
	}
	...
	public boolean isTokenValid(String token) {
		try {
			Claims claims = Jwts.parser().verifyWith(secretKey) // 서명 검증 설정
